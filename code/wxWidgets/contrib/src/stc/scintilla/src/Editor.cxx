...
void Editor::LayoutLine(int line, Surface *surface, ViewStyle &vstyle, LineLayout *ll, int width) {
	...
	if (ll->validity == LineLayout::llInvalid) {
		...
		ll->styleBitsSet = 0;
		bool collapsed = !cs.GetExpanded(line);
		...
		for (int charInDoc = posLineStart; charInDoc < posLineEnd; charInDoc++) {
			...
			if ((collapsed && (foldFlags & SC_FOLDFLAG_ELLIPSIS)) || vstyle.viewEOL || (!IsEOLChar(chDoc))) {

				if (collapsed && (foldFlags & SC_FOLDFLAG_ELLIPSIS) && chDoc == '\n' && charInDoc > posLineStart && pdoc->CharAt(charInDoc-1) == '\r') {
					// nothing to do
				} else {
					ll->chars[numCharsInLine] = chDoc;
			...
			if ((ll->styles[charInLine] != ll->styles[charInLine + 1]) ||
				...
				if (vstyle.styles[ll->styles[charInLine]].visible) {
					if (isControl) {
						...
						} else if (collapsed && (foldFlags & SC_FOLDFLAG_ELLIPSIS) && ll->chars[charInLine] == '\n' && charInLine > 0 && ll->chars[charInLine - 1] == '\r') {
							ll->positions[charInLine + 1] = 0;
						} else if (collapsed && (foldFlags & SC_FOLDFLAG_ELLIPSIS) && (ll->chars[charInLine] == '\r' || ll->chars[charInLine] == '\n')) {
							ll->positions[charInLine + 1] = 21;
...
void Editor::DrawLine(Surface *surface, ViewStyle &vsDraw, int line, int lineVisible, int xStart,
	...
	for (i = lineStart; twoPhaseDraw && (i < lineEnd); i++) {
		...
		if ((ll->styles[i] != ll->styles[i + 1]) ||
			...
			if ((rcSegment.left <= rcLine.right) && (rcSegment.right >= rcLine.left)) {
				...
				} else if (collapsed && (foldFlags & SC_FOLDFLAG_ELLIPSIS) && ll->chars[i] == '\n' && i > lineStart && ll->chars[i - 1] == '\r') {
					// nothing to do
				} else if ((collapsed && (foldFlags & SC_FOLDFLAG_ELLIPSIS)) && (ll->chars[i] == '\n' || ll->chars[i] == '\r')) {
					inIndentation = false;
					surface->FillRectangle(rcSegment, textBack);
	...
	for (i = lineStart; i < lineEnd; i++) {
		...
		if ((ll->styles[i] != ll->styles[i + 1]) ||
			...
			if ((rcSegment.left <= rcLine.right) && (rcSegment.right >= rcLine.left)) {
               ...
               PRectangle rcEllipsis = rcSegment;
               
               int ellipsisWidth = surface->WidthText( vsDraw.styles[STYLE_INDENTGUIDE].font, ellipsis, strlen( ellipsis ) );

               ColourAllocated ellipsisFore = vsDraw.styles[STYLE_INDENTGUIDE].fore.allocated;
				   if (inSelection && (vsDraw.selforeset)) {
					   ellipsisFore = vsDraw.selforeground.allocated;
				   }

               // rectangle
					rcEllipsis.right = rcEllipsis.left + ellipsisWidth + 2;
					surface->RectangleDraw(rcEllipsis, ellipsisFore, textBack);

					// ellipsis
               rcEllipsis.left += 1;
               surface->DrawTextTransparent( rcEllipsis, 
                  vsDraw.styles[STYLE_INDENTGUIDE].font,
                  ...
                  ellipsisFore );
					...
					/*
               ++rcEllipsis.left;
					rcEllipsis.top += vsDraw.maxAscent - 4;
					rcEllipsis.bottom = rcEllipsis.top + 2;
					for (int i = 0; i < 3; ++i) {
						rcEllipsis.left   = rcEllipsis.left + (i == 0 ? 3 : 4);
						rcEllipsis.right  = rcEllipsis.left + 2;
						surface->FillRectangle(rcEllipsis, vs.styles[STYLE_INDENTGUIDE].fore.allocated);
					}
               */
	...
	if (vsDraw.edgeState == EDGE_LINE) {
		int edgeX = theEdge * vsDraw.styles[STYLE_INDENTGUIDE].spaceWidth;
...
void Editor::RefreshPixMaps(Surface *surfaceWindow) {
	...
	if (!pixmapIndentGuide->Initialised()) {
      ...
      pixmapIndentGuide->FreePen();
      pixmapIndentGuideHighlight->FreePen();
	}
